...
std.get('/student-route', (req, res) => {
    ...
    SELECT s.Std_Id, s.Std_Name, s.Std_Email, s.Std_Department, Std_Mobile, Std_Profile_Url,
           ...
           q.Query_Id, q.Queries, q.Query_CT, 
           r.Response_Id, r.Response, r.Response_CT,
           (SELECT COUNT(*) FROM Query WHERE Std_Id = s.Std_Id) AS total_queries,
           (SELECT COUNT(*) FROM Query q LEFT JOIN Response r ON q.Query_Id = r.Query_Id WHERE q.Std_Id = s.Std_Id AND r.Response_Id IS NOT NULL) AS resolved_queries,
           (SELECT COUNT(*) FROM Query q LEFT JOIN Response r ON q.Query_Id = r.Query_Id WHERE q.Std_Id = s.Std_Id AND r.Response_Id IS NULL) AS unresolved_queries
    FROM student AS s 
    JOIN mentor AS m ON s.Mentor_Id = m.Mentor_Id 
    LEFT JOIN Query AS q ON s.Std_Id = q.Std_Id 
    LEFT JOIN Response AS r ON q.Query_Id = r.Query_Id 
